...
std::atomic<size_t> events_to_process = 0;
...
int pinit() {
  ...
  std::lock_guard lock(waveform_mutex);
...
int process_event(Event *e) {
  ...
  for (int i = 0; i < WAVEFORM_SIZE; i++) {
    ...
    // time_t now;
    // time(&now);
    // waveform_histogram->SetBinContent(i + 1, now % 10);
    
    // std::cout << std::setw(5) << std::fixed << std::setprecision(0) << value;
    if (std::abs(value) > std::abs(peak_value))
      peak_value = value;
  }
